    ...
    while True:
        ...
        if end_idx != -1:
            if norm_found_path in target_files:
                ...
                # Check if this file was already injected into a system instruction
                # If so, we can deduplicate here to save tokens.
    ...
    for msg in messages:
        if msg.get("role") == "system":
            ...
            for path in target_files.keys():
                # We check if the marker for this file exists in the system message
                # We use a broad check for the marker prefix
                if "--- Context from: " in content:
                    # More precise check: find the actual path in the marker
                    # This is a bit simplified but usually works
                    pass
            
            # Actually, we can just use a regex to find all "Context from" paths in the system message
...
def run_hook():
    try:
        ...
        # 1. Identify files already present in system instructions
        files_in_system = get_files_in_system_instructions(messages, target_files)
        if system_instruction:
            # Also check the separate systemInstruction field if it exists
            content = ""
        ...
        # 2. Update systemInstruction field if it exists
        if system_instruction:
            new_si, si_updated = update_message_object(system_instruction, target_files, set()) # Don't deduplicate SI itself
            if si_updated:
        ...
        for msg in messages:
            # For system messages, we don't deduplicate. 
            # For user/model messages, we deduplicate if already seen in a system message.
